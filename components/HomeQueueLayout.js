...
import { Spinner, Text, View, Animated } from "native-base";
...
const updateSoc = async () => {
  try {
    ...
    const response = await fetch(
      `https://api.connect-business.net/fleet/v1/fleets/1A3D13CCC6694F03ADBC1BC6CFADCB4B/vehicles.dynamic/${selectedCar.vin}`,
      options
    );
...
const HomeQueueLayout = (props) => {
  ...
  const { soc, fetchSoc } = useApiHooks();
  ...
  return (
    ...
    >
      ...
      <View style={{ display: "flex", justifyContent: "center", flex: 8 }}>
        <BatteryInfo
          batteryStatus={selected.soc}
          sizeVariable="large"
          charging={parkingSpots.inSpot}
        />
        {parkingSpots.inSpot ? (
          <Text style={styles.estimatedText}>Estimated time: TODO</Text>
        ) : null}
      </View>
      <View style={{ flex: 1 }}>
